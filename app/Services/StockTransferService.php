    ...
    public function createDraft(array $data, array $items, int $userId): StockTransfer
    {
        if ($data['source_warehouse_id'] === $data['destination_warehouse_id']) {
            throw ValidationException::withMessages([
                'destination_warehouse_id' => 'Origin and destination warehouses must be different.',
            ]);
        }
        ...
        DB::transaction(function () use ($transfer, $userId) {
            ...
            foreach ($transfer->items as $item) {
                ...
                if ($available < $item->qty) {
                    throw ValidationException::withMessages([
                        'transfer' => "Stock for {$item->product->title} is insufficient in the source warehouse (available: {$available}).",
                    ]);
                }
            }
            ...
            foreach ($transfer->items as $item) {
                ...
                StockMutation::create([
                    ...
                    'notes' => 'Transfer to '.$transfer->destinationWarehouse->code,
                    'created_by' => $userId,
                ]);
            }
            ...
            $this->auditLogService->log(
                ...
                module: 'stock',
                auditable: $transfer,
                description: 'Stock transfer '.$transfer->document_number.' sent.',
        ...
        DB::transaction(function () use ($transfer, $userId) {
            ...
            foreach ($transfer->items as $item) {
                ...
                StockMutation::create([
                    ...
                    'notes' => 'Transfer from '.$transfer->sourceWarehouse->code,
                    'created_by' => $userId,
                ]);
            }
            ...
            $this->auditLogService->log(
                ...
                module: 'stock',
                auditable: $transfer,
                description: 'Stock transfer '.$transfer->document_number.' received.',
                before: ['status' => $before->status],
    ...
    public function cancel(StockTransfer $transfer, int $userId): void
    {
        ...
        DB::transaction(function () use ($transfer) {
            ...
            $this->auditLogService->log(
                ...
                module: 'stock',
                auditable: $transfer,
                description: 'Stock transfer '.$transfer->document_number.' cancelled.'.($returnStock ? ' Stock was returned to the source warehouse.' : ''),
                before: ['status' => $before->status],
