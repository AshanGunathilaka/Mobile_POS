...
class CrmAutomationService
{
    ...
    public function audienceOptions(): array
    {
        return [
            ...
            'receivable_statuses' => [
                ['value' => 'all', 'label' => 'All Receivable Statuses'],
                ['value' => 'has_receivable', 'label' => 'Has Receivable'],
                ['value' => 'overdue', 'label' => 'Overdue Receivable'],
                ['value' => 'due_soon', 'label' => 'Due in 3 Days'],
            ],
            'voucher_filters' => [
                ['value' => 'all', 'label' => 'All Customers'],
                ['value' => 'has_active_voucher', 'label' => 'Has Active Voucher'],
                ['value' => 'no_active_voucher', 'label' => 'No Active Voucher'],
            ],
    ...
    public function createInvoiceShareCampaignForReceivable(Receivable $receivable, int $userId): CustomerCampaign
    {
        $campaign = CustomerCampaign::query()->firstOrCreate(
            ...
            [
                'name' => 'Share Receivable '.$receivable->invoice,
                'type' => CustomerCampaign::TYPE_INVOICE_SHARE,
                'status' => CustomerCampaign::STATUS_READY,
                'channel' => CustomerCampaign::CHANNEL_WHATSAPP_LINK,
                ...
                'message_template' => 'Invoice {{invoice}} total {{remaining}} due on {{due_date}}',
                'processed_at' => now(),
                'created_by' => $userId,
            ]
        );
        ...
        $shareText = sprintf(
            'Receivable reminder %s. Remaining balance LKR %s. Due date: %s',
            $receivable->invoice,
    ...
    private function generateDueSoonCampaign(CarbonInterface $at): void
    {
        ...
        $campaign = CustomerCampaign::query()->firstOrCreate(
            ...
            [
                ...
                'message_template' => 'Reminder: invoice {{invoice}} is due on {{due_date}}',
                'processed_at' => $at,
            ]
        );
        ...
        if ($campaign->wasRecentlyCreated) {
            ...
            $this->fillReceivableReminderLogs($campaign, $receivables, 'due date');
        }
    }
    ...
    private function generateOverdueCampaign(CarbonInterface $at): void
    {
        ...
        $campaign = CustomerCampaign::query()->firstOrCreate(
            ['context_key' => $contextKey],
            [
                'name' => 'Overdue Receivable Reminder '.$at->format('d M Y'),
                'type' => CustomerCampaign::TYPE_DUE_DATE_REMINDER,
                'status' => CustomerCampaign::STATUS_READY,
                'channel' => CustomerCampaign::CHANNEL_INTERNAL,
                'audience_filters' => ['receivable_status' => 'overdue'],
                'message_template' => 'Receivable {{invoice}} has been overdue since {{due_date}}',
                'processed_at' => $at,
            ]
        );
    ...
    private function generateRepeatOrderCampaign(CarbonInterface $at): void
    {
        ...
        $campaign = CustomerCampaign::query()->firstOrCreate(
            ...
            [
                ...
                'message_template' => 'Customer has not purchased recently. Invite them to shop again.',
                'processed_at' => $at,
            ]
        );
        ...
        if ($campaign->wasRecentlyCreated) {
            ...
            foreach ($customers as $customer) {
                $message = 'Hello '.$customer->name.', we miss your visit. Come shop with us again today.';
    ...
    private function fillReceivableReminderLogs(CustomerCampaign $campaign, Collection $receivables, string $reason): void
    {
        foreach ($receivables as $receivable) {
            $message = sprintf(
                'Reminder %s for invoice %s. Remaining balance LKR %s. Due date %s.',
                $reason,
                $receivable->invoice,
                number_format($receivable->remaining, 2, '.', ','),
                optional($receivable->due_date)?->format('d/m/Y') ?? '-'
    ...
    {
        $template = $campaign->message_template ?: 'Hello {{name}}, there is a special promo for you.';
        $message = str_replace(
            ['{{name}}', '{{phone}}'],
