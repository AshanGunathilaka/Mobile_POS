...
class PricingService
{
    ...
    public function ruleLabel(PricingRule $rule): string
    {
        return match ($rule->kind) {
            PricingRule::KIND_QTY_BREAK => 'Wholesale '.$this->standardDiscountLabel($rule),
            PricingRule::KIND_BUNDLE_PRICE => 'Bundle LKR '.number_format((float) $rule->discount_value, 2, '.', ','),
    ...
    private function standardDiscountLabel(PricingRule $rule): string
    {
        return match ($rule->discount_type) {
            ...
            PricingRule::TYPE_FIXED_AMOUNT => 'Save LKR '.number_format((float) $rule->discount_value, 2, '.', ','),
            PricingRule::TYPE_FIXED_PRICE => 'Price LKR '.number_format((float) $rule->discount_value, 2, '.', ','),
            default => $rule->name,
        };
    }
