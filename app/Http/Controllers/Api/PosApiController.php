...
class PosApiController extends Controller
{
    ...
    public function closeShift(Request $request): JsonResponse
    {
        ...
        if (! $shift) {
            return $this->error('No active shift.', 422);
        }
    ...
    public function scan(Request $request): JsonResponse
    {
        ...
        if (! $product) {
            return $this->notFound('No product was found for that barcode.');
        }
        ...
        return $this->ok($data, 'Product found');
    }
    ...
    public function addToCart(Request $request): JsonResponse
    {
        ...
        if ($product->is_composite) {
            ...
            foreach ($product->components as $component) {
                ...
                if ($avail < $needed) {
                    return $this->error("Stock for {$component->title} is insufficient.", 422);
                }
            }
            ...
            if ($availableStock < $baseQty) {
                return $this->error("Insufficient stock. Available: {$availableStock}", 422);
            }
    ...
    public function updateCart(Request $request, Cart $cart): JsonResponse
    {
        ...
        if ($cart->cashier_id !== $request->user()->id) {
            return $this->forbidden('This cart does not belong to you.');
        }
    ...
    public function holdCart(Request $request): JsonResponse
    {
        ...
        if ($activeCarts->isEmpty()) {
            return $this->error('The cart is empty; nothing can be held.', 422);
        }
        ...
        return $this->ok(['hold_id' => $holdId, 'label' => $label], 'Transaction held.');
    }
    ...
    public function resumeHold(Request $request, string $holdId): JsonResponse
    {
        ...
        if ($activeCount > 0) {
            return $this->error('Complete or hold the active transaction first.', 422);
        }

        $held = Cart::where('cashier_id', $userId)->forHold($holdId)->get();
        if ($held->isEmpty()) {
            return $this->notFound('Held transaction not found.');
        }
        ...
        return $this->ok(null, 'Transaction resumed.');
    }
    ...
    public function checkout(Request $request, PaymentGatewayManager $paymentGatewayManager): JsonResponse
    {
        ...
        if ($isPayLater && ! $request->filled('due_date')) {
            return $this->error('Due date is required for pay-later invoices.', 422);
        }
    ...
    public function transactionDetail(Request $request, Transaction $transaction): JsonResponse
    {
        if (! $request->user()->isSuperAdmin() && $transaction->cashier_id !== $request->user()->id) {
            return $this->forbidden('This transaction does not belong to you.');
        }
