...
class TransactionController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($product->is_composite) {
            ...
            foreach ($product->components as $component) {
                ...
                if ($avail < $needed) {
                    return redirect()->back()->with('error', "Insufficient stock for {$component->title}.");
                }
            }
    ...
    public function holdCart(Request $request)
    {
        ...
        if ($activeCarts->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'Cart is empty; there is nothing to hold.',
            ], 422);
        }

        // Generate unique hold ID
        $holdId = 'HOLD-'.strtoupper(uniqid());
        $label = $request->label ?: 'Transaction '.now()->format('H:i');
        ...
        return back()->with('success', 'Transaction held: '.$label);
    }
    ...
    public function resumeCart($holdId)
    {
        ...
        if ($activeCarts > 0) {
            return response()->json([
                'success' => false,
                'message' => 'Complete or hold the active transaction first.',
            ], 422);
        }
    ...
    public function clearHold($holdId)
    {
        ...
        if ($deleted === 0) {
            return request()->wantsJson()
                ...
                : back()->with('error', 'Held transaction not found.');
        }
        ...
        return back()->with('success', 'Held transaction deleted successfully.');
    }
    ...
    public function store(Request $request, PaymentGatewayManager $paymentGatewayManager)
    {
        ...
        if ($isPayLater && ! $request->filled('due_date')) {
            return redirect()
                ...
                ->with('error', 'Due date is required for pay later transactions.');
        }
        ...
        if ($transaction->discount > 0 && $transaction->needsDiscountApproval()) {
            ...
            return redirect()
                ...
                ->with('info', 'Transaction is waiting for supervisor approval.');
        }
    ...
    public function confirmPayment(Transaction $transaction)
    {
        if ($transaction->payment_status === 'paid') {
            return redirect()
                ->back()
                ->with('error', 'Transaction has already been paid.');
        }
        ...
        $this->auditLogService->log(
            ...
            description: "Payment for invoice {$transaction->invoice} confirmed.",
            before: [
        ...
        return redirect()
            ->back()
            ->with('success', "Payment for invoice {$transaction->invoice} confirmed successfully.");
    }
}
