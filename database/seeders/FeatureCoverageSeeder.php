...
class FeatureCoverageSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $this->command?->info('Seeding feature coverage records...');
    ...
    private function resetFeatureData(): void
    {
        ...
        AuditLog::query()
            ...
            ->where(function ($query) {
                $query
                    ->where('description', 'like', 'Stock added from goods receiving %')
                    ->orWhere('description', 'like', 'Stock out from supplier return %')
                    ->orWhere('description', 'Stock opname difinalisasi.%')
                    ->orWhere('description', 'like', 'Stock product disesuaikan melalui stock opname.%');
            })
            ->delete();
    ...
    private function seedStoreSettings(): void
    {
        $settings = [
            ...
            'store_email' => ['value' => 'halo@majubersama.test', 'description' => 'Store email'],
    ...
    private function seedPurchaseFlow(
        ...
        $cancelledOrder = $purchaseOrderService->createOrder(
            [
                ...
                'notes' => 'Purchase order cancelled because the supplier price changed.',
            ],
            [
                [
        ...
        $partialOrder = $purchaseOrderService->createOrder(
            [
                ...
                'notes' => 'Purchase order for fast-moving items for weekly restock.',
            ],
            [
                [
        ...
        $completedReceiving = $goodsReceivingService->receive(
            ...
            $completedOrder->items->map(fn (PurchaseOrderItem $item) => [
                ...
                'notes' => 'Fully received from supplier.',
            ])->all(),
            'All items were received in full and moved directly into the warehouse.',
            $cashier->id,
        );
        ...
        $draftReturn = $supplierReturnService->createReturn(
            [
                ...
                'payable_id' => $completedPayable?->id,
                'notes' => 'Draft return for dented yogurt, waiting for supplier approval.',
            ],
            [
                [
                    ...
                    'notes' => 'Not processed yet, still waiting for pickup.',
                ],
            ],
            $admin->id,
        );
        ...
        $completedReturn = $supplierReturnService->createReturn(
            [
                ...
                'goods_receiving_id' => $completedReceiving->id,
                'payable_id' => $completedPayable?->id,
                'notes' => 'Return of damaged items from the frozen food batch.',
            ],
            [
                [
    ...
    private function seedStockOpnames(User $admin, Collection $products): void
    {
        ...
        $draftOpname = StockOpname::create([
            'code' => 'SO-DRAFT-001',
            'status' => 'draft',
            'notes' => 'Front shelf stock count session, not all items counted yet.',
            'created_by' => $admin->id,
        ...
        $finalizedOpname = StockOpname::create([
            'code' => 'SO-FINAL-001',
            'status' => 'draft',
            'notes' => 'Cold storage stock count for the early-week batch.',
            'created_by' => $admin->id,
        ...
        $adjustments = collect([
            ...
            [
                ...
                'physical_stock' => null,
                'adjustment' => 2,
                'reason' => 'Temuan stock terselip di rak pendingin.',
            ],
            [
    ...
    private function seedSupplementaryAuditLogs(User $admin, User $cashier, Collection $bankAccounts): void
    {
        ...
        $auditLogService->log(
            ...
            description: 'Supervisor meninjau ringkasan shift cashier aktif.',
            meta: [
