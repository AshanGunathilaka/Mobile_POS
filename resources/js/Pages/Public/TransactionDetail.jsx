...
const formatPrice = (v = 0) => Number(v || 0).toLocaleString("en-LK", { style: "currency", currency: "LKR", minimumFractionDigits: 2 });
const formatDate = (v) => v ? new Date(v).toLocaleString("en-LK", { dateStyle: "full", timeStyle: "short" }) : "-";
                ...
                <div className="max-w-lg mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="border-t border-slate-100 pt-4 space-y-1.5 text-sm">
                            ...
                            {transaction.discount > 0 && <div className="flex justify-between text-slate-500"><span>Discount</span><span>-{formatPrice(transaction.discount)}</span></div>}
                            {transaction.tax_total > 0 && <div className="flex justify-between text-slate-500"><span>Tax</span><span>{formatPrice(transaction.tax_total)}</span></div>}
                        ...
                        {transaction.payment_method === "cash" && transaction.cash > 0 && (
                            <div className="border-t border-slate-100 pt-4 space-y-1.5 text-sm">
                                <div className="flex justify-between text-slate-500"><span>Cash</span><span>{formatPrice(transaction.cash)}</span></div>
                                {transaction.change > 0 && <div className="flex justify-between text-slate-500"><span>Back</span><span>{formatPrice(transaction.change)}</span></div>}
                            </div>
                        )}

                        {transaction.receivable && transaction.receivable.status !== "paid" && (
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
                                <p className="text-sm font-semibold text-amber-800">Remaining Payment</p>
                                <div className="flex justify-between text-sm">
                                    <span className="text-amber-700">Due date: {formatDate(transaction.receivable.due_date)}</span>
