            ...
            <form onSubmit={submit} className="max-w-5xl">
                <div className="space-y-6">
                                ...
                                {filteredProducts.map((product) => (
                                    <button key={product.id} type="button" onClick={() => addItem(product)} className="flex w-full items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-4 py-3 text-left text-sm transition hover:border-primary-200 hover:bg-primary-50 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-primary-700 dark:hover:bg-primary-950/20">
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">{product.sku || "-"} &bull; Stock: {product.stock}</p>
                                        </div>
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className="flex justify-end gap-3">
                        <Link href={route("stock-transfers.index")} className="flex h-11 items-center rounded-xl border border-slate-200 bg-white px-6 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">Cancel</Link>
                        <Button type="submit" icon={<IconPlus size={18} />} className="bg-primary-500 hover:bg-primary-600 text-white shadow-lg shadow-primary-500/30" label={processing ? "Saving..." : "Save Draft"} disabled={processing} />
                    </div>
                </div>
            </form>
        </>
