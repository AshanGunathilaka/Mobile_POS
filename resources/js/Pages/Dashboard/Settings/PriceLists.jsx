...
export default function PriceLists({ priceLists }) {
    ...
    const scopeLabel = { all: "All", walk_in: "Walk-in", registered: "Registered", member: "Member", segment: "Segment" };
            ...
            <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
                            ...
                            <div className="flex gap-3">
                                <button type="submit" className="px-4 py-2 rounded-xl bg-primary-500 text-white text-sm font-medium">{editing ? "Update" : "Save"}</button>
                                <button type="button" onClick={resetForm} className="px-4 py-2 rounded-xl border text-sm font-medium">Cancel</button>
                            </div>
                        </form>
                    </div>
                )}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    {priceLists.length > 0 ? (
                            ...
                            {priceLists.map(pl => (
                                <div key={pl.id} className="p-4 flex items-center gap-4">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-slate-500">{scopeLabel[pl.customer_scope] || pl.customer_scope} • {pl.items_count} product • Prioritas {pl.priority}</p>
                                    </div>
                        ...
                        <div className="p-8 text-center text-slate-400">No price lists yet.</div>
                    )}
                </div>
            </div>
        </>
