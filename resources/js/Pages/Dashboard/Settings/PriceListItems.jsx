...
const formatPrice = (v = 0) => Number(v).toLocaleString("en-LK");
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">{priceList.items.length} product</p>
                    </div>
                </div>

                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                    <h3 className="font-semibold mb-3">Add Price</h3>
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search product..." className="h-11 w-full rounded-xl border border-slate-200 px-4 text-sm mb-3" />
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    {priceList.items.length > 0 ? (
                            ...
                            {priceList.items.map(item => (
                                <div key={item.id} className="p-4 flex items-center gap-4">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-slate-500">{item.product?.sku || "-"} • Price normal: {formatPrice(item.product?.sell_price)}</p>
                                    </div>
                        ...
                        <div className="p-8 text-center text-slate-400">No items yet. Search products above to add one.</div>
                    )}
                </div>
            </div>
        </>
