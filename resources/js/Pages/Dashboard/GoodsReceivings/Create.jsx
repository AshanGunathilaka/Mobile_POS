...
export default function Create({ orders }) {
    ...
    const submit = (e) => {
        ...
        if (!data.purchase_order_id) {
            toast.error("Select purchase order terlebih dahulu.");
            return;
        }
            ...
            <form onSubmit={submit} className="max-w-4xl">
                <div className="space-y-6">
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                        <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-white">Select Purchase Order</h2>
                        <select
                            value={selectedPoId}
                    ...
                    {selectedOrder && data.items.length > 0 && (
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                            ...
                            <textarea
                                ...
                                placeholder="Goods receiving notes (optional)"
                            />
                        </div>
                    )}
